...
static hash_head_t file_hash[FILE_HASH_SIZE + 1] = {
    0,
};
...
void lock_hash() {
  MUTEX_LOCK(&file_hash[FILE_HASH_SIZE].mutex);
}

void unlock_hash() {
  MUTEX_UNLOCK(&file_hash[FILE_HASH_SIZE].mutex);
}
...
mmio_t *put_mmio_hash(unsigned long ino, mmio_t *mmio) {
  ...
  if (!list_empty(&file_hash[index].head)) {
    LIST_FOR_EACH_ENTRY(hnode, &file_hash[index].head, list) {
      ...
      if (ino == tmp_mmio->ino) {
        ...
        MUTEX_UNLOCK(&file_hash[index].mutex);
        tmp_mmio->ref++;
        PRINT("tmp_mmio = %p tmp_mmio->ref = %d\n",tmp_mmio, tmp_mmio->ref);
        return tmp_mmio;
      }
    }
  }
  ...
  PRINT("mmio = %p mmio->ref = %d",mmio, mmio->ref);
  /* Unlock the list. */
  MUTEX_UNLOCK(&file_hash[index].mutex);
  return mmio;
}
