...
#define ALLOC_TABLE(table, type, parent_, index_)                                \
  do {                                                                           \
    table = alloc_log_table(type);                                               \
    table->parent = parent_;                                                     \
    table->index = index_;                                                       \
    table->offset = parent_->offset + index_ * TABLE_SIZE[type];                 \
    if (!__sync_bool_compare_and_swap(&parent_->entries[index_], NULL, table)) { \
      free_log_table(table);                                                     \
      table = parent_->entries[index_];                                          \
    }                                                                            \
  } while (0)                                                                 
