...
#define num_time 5
struct timeval tv_begin[num_time],tv_end[num_time];
long long call_number[num_time];
long long runtime_us[num_time];
char address[10][255];
int address_index = 0;
...
static void *mmap_logfile(const char *path, size_t len) {
  ...
  PRINT("file=%s, len=%s, addr=%p", path, get_readable_size(len), addr);
  sprintf(address[address_index++],"file=%s, len=%lu, addr=%p\n", path, len, addr);
  return addr;
}
...
static void init_table(void *obj) { 
  log_table_t *entry;
  memset(obj, 0, sizeof(log_table_t)); 
  entry = (log_table_t *)obj;
...
static void create_global_table_list(void) {
  ...
  addr = alloc_pmem("tables", 0, mem_size);
...
static void init_idx(void *obj) {
  idx_entry_t *entry;
  memset(obj, 0, sizeof(idx_entry_t)); 
...
static void init_mmio(void *obj) {
  ...
  mmio->policy_g = DEFAULT_RW_POLICY;
  mmio->policy_l = DEFAULT_LOCK_POLICY;
...
mmio_t *get_new_mmio(int fd, int flags, unsigned long ino,
  ...
  Log* blog = (Log*)mmio->bitmaplog.log;
...
void release_mmio(mmio_t *mmio, int flags, int fd) {
  ...
  free_log_data(mmio->bitmaplog.log,LOG_4K); 
  init_mmio(mmio);
  PRINT("initial the mmio");
...
void free_log_table(log_table_t *table) {
  memset(table, 0, sizeof(log_table_t)); 
