...
#define LOCK_ENTRIES(type_, mmio_, offset_, len_, entries_head_)         \
  do {                                                                   \
    ...
    while (n_ > 0) {                                                     \
      ...
      PRINT("log_size=%lu off=%d", LOG_SIZE(log_size_), off_);           \
      index_ = TABLE_INDEX(log_size_, off_);                             \
      PRINT("table = %p TABLE index=%lu", table_, index_);               \
                                                                         \
      do {                                                               \
        entry_ = get_log_entry(mmio_->epoch, table_, index_, log_size_); \
        PRINT("entry_=%p", entry_);                                      \
...
static void expend_mmio(mmio_t *mmio, int fd, off_t offset, size_t len) {
  ...
  //printf("bravo_write_lock(&mmio->rwlock);\n");
  //RWLOCK_UNLOCK(&mmio->rwlockp);
  //RWLOCK_WRITE_LOCK(&mmio->rwlockp);
  ...
  log_size_t log_size;
  void *dst, *src;
  unsigned long i, current_epoch, endoff;
          ...
          if (entry && entry->epoch < current_epoch) {
            if (RWLOCK_WRITE_TRYLOCK(entry->rwlockp)) {
              if (entry->epoch < current_epoch) {
                ...
                PRINT("clear the idx_entry: offset=%lu, table idx=%lu", offset,i);
                continue;
              }
...
void checkpoint_mmio(mmio_t *mmio) {
  ...
  PRINT("mmio->running = %d\n",mmio->running);
  //if (!__sync_bool_compare_and_swap(&mmio->running,0,1)) return;

  PRINT("start checkpointing: mmio->ino=%lu", mmio->ino);
  offset = 0;
  endoff = mmio->fsize;
  if (mmio->fsize > mmio->end - mmio->start)
    HANDLE_ERROR("Too larger file size");
  current_epoch = mmio->epoch;
  PRINT("mmio->start = %p endoff = %d\n",mmio->start, endoff);
  while (offset < endoff) {
    log_size = LOG_4K;
    if (bravo_read_trylock(&mmio->rwlock) == 0) {
      ...
      if (table) {
        log_size = table->log_size;
        PRINT("log_size = %d entry_num = %d",log_size,NR_ENTRIES(log_size));
        int max_index = offset + 1 << 21 <= endoff ? (int)NR_ENTRIES(log_size) : (endoff - offset) / LOG_SIZE(log_size);
        if ((endoff-offset)%LOG_SIZE(log_size)) max_index++;
        if (max_index > NR_ENTRIES(log_size))
          max_index = NR_ENTRIES(log_size);
        PRINT("max_index = %d\n",max_index);
        for (int i = 0; i < max_index; i++) {
          if (offset + i * LOG_SIZE(log_size) >= mmio->fsize)
            break;
          entry = table->entries[i];
          PRINT("entry = %p i = %d entry_num = %d",entry,i,(int)NR_ENTRIES(log_size));
          if (entry && entry->len && entry->epoch < current_epoch) {
            PRINT("i = %d entry->len = %d entry->epoch = %d", i, entry->len,entry->epoch);
            if (RWLOCK_WRITE_TRYLOCK(entry->rwlockp)) {
              if (entry->epoch < current_epoch) {
                if (entry->policy == REDO) {
                  dst = entry->dst + entry->offset;
    ...
    offset += 1 << 21;
    //offset += NR_ENTRIES(log_size) * LOG_SIZE(log_size);
    //printf("%d %d\n",1 << 21 ,NR_ENTRIES(log_size) * LOG_SIZE(log_size));
    //PRINT("offset = %d",offset);
  }
  //__sync_lock_release(&mmio->running);
  PRINT("complete checkpointing mmio->ino=%lu\n", mmio->ino);
}
...
ssize_t mmio_write(mmio_t *mmio, int fd, off_t offset, const void *buf,
   ...
   */
  idx_entry_t *entry_tmp;  
  SLIST_HEAD(entries_head);
  LOCK_ENTRIES(wr, mmio, offset, len, old_entries_head);
  while (slist_empty(&old_entries_head) == 0) {
    entry_tmp = slist_pop(&old_entries_head);
    slist_push(entry_tmp, &entries_head);
  }
  ...
  SLIST_FOR_EACH_ENTRY(entry, &entries_head, list) {
    PRINT("entry = %p entry->log = %p", entry, entry->log);
    if (entry->epoch < mmio->epoch && entry->len) {
    ...
    PRINT("log_size = %d, log_offset = %d, log_len = %d, len = %d",log_size, log_offset, log_len, len);
    if ((unsigned long)len > log_len) {
      n = log_len;
    } else {
      n = len;
    }
...
ssize_t mmio_read(mmio_t *mmio, off_t offset, void *buf, size_t len) {
   ...
   */
  LOCK_ENTRIES(rd, mmio, offset, len, old_entries_head);
  idx_entry_t *entry_;  
  SLIST_HEAD(entries_head);

  while (slist_empty(&old_entries_head) == 0) {
    entry_ = slist_pop(&old_entries_head);
    slist_push(entry_, &entries_head);
  }
